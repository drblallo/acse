...
{
    /* sign extension of a 16 bit value over 32 bit */
    instr->imm  = (instr->imm & 0xffff);
    if (instr->imm & 0x8000)
        instr->imm |= 0xffff0000;

    /* sign extension of a 20 bit value over 32 bit */
    instr->addr  = instr->addr & 0x0fffff;
    if (instr->addr & 0x080000)
        instr->addr |= 0xfff00000;
}
